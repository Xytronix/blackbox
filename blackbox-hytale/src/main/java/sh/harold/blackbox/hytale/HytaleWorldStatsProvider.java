...
/**
 * Supplies per-world tick health (TPS/MSPT) and player counts for the health snapshot,
 * using the same metrics Hytale's own /world perf command reads.
 */
final class HytaleWorldStatsProvider implements WorldStatsProvider {
    ...
    /**
     * {p50, p95, max} tick length in milliseconds from the 5m window's raw samples;
     * {-1, -1, -1} when the window is empty.
     */
    ...
    /**
     * Average raw network round-trip across the world's players, in milliseconds. Reads the
     * 1-second average of each player's {@code PongType.Raw} ping metric (recorded in
     * microseconds by the engine); -1 when no player reports one.
     */
