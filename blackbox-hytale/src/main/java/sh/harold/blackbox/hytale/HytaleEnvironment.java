...
/**
 * Builds an "Environment" diagnostics section: which loaders (Hyinit, Hyxin) are installed, with
 * versions where resolvable, plus the full installed-mod list. Only installed tooling is listed —
 * nothing references a loader that isn't actually present.
 */
final class HytaleEnvironment {
    ...
    /**
     * Which container runtime hosts this JVM: "Kubernetes", "Docker", "Podman", "Container"
     * (cgroup-managed but unidentified), or null on bare metal. The probed paths only exist on
     * Linux, so non-container hosts simply resolve null.
     */
    ...
    /** Returns "installed" / "installed (version)" when any candidate class is present, else null. */
    ...
    /**
     * Version of the loader that owns {@code className}. Hyinit and Hyxin record their version
     * ONLY in the templated {@code manifest.json} at the root of their jar (never in the
     * META-INF manifest), and a bootstrapper's classes may load with a null or unhelpful
     * ProtectionDomain, so this tries in order: the Implementation-Version attribute, the
     * manifest.json inside the class's code-source jar, a scan of {@code earlyplugins/*.jar},
     * the jar behind the class's resource URL, and finally the entry jar from the launch
     * command. Null when unresolvable.
     */
    ...
    /**
     * Version of the loader when it runs as a {@code -javaagent:} jar (how Hyinit is deployed on
     * agent-launched servers): the agent jar whose filename carries the loader's hint, else the
     * first agent jar whose manifest.json "Name" matches the loader.
     */
    ...
    /** Resolves the jar behind the class's "jar:file:...!/..." resource URL and reads its version. */
    ...
    /**
     * Version of the entry jar from {@code java -jar <loader>.jar}: its manifest.json, or the
     * version embedded in the jar filename as a last resort.
     */
    ...
    /** "Version" field of the Hytale manifest.json at the root of {@code jar}, or null. */
    private static String manifestJsonVersion(java.nio.file.Path jar) {
        return manifestJsonField(jar, MANIFEST_VERSION);
    }

    /** "Name" field of the Hytale manifest.json at the root of {@code jar}, or null. */
    ...
    /** Scans earlyplugins/*.jar for the jar containing {@code className} and reads its version. */
