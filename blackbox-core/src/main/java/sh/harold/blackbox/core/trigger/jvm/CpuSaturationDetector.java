...
/**
 * Emits a CPU-saturation trigger event when the process CPU load stays at or above a threshold
 * for a sustained period. Process load (not machine load) so a noisy neighbour on the host does
 * not blame the server. Edge-triggered with hysteresis: fires once per episode and re-arms after
 * the load recovers below {@link #REARM_FACTOR} times the threshold.
 */
public final class CpuSaturationDetector {

    private static final double REARM_FACTOR = 0.9;

    /** Supplies the process CPU load (0..1), or a negative value when unknown. */
    @FunctionalInterface
