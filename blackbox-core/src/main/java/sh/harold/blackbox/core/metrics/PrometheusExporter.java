...
public final class PrometheusExporter implements AutoCloseable {
    ...
    public static String render(HealthGauges gauges) {
        ...
        counterL(sb, "blackbox_gc_pause_msec_total", "Cumulative stop-the-world GC pause time, in milliseconds.", gauges.gcPauseMsec());
        counterL(sb, "blackbox_allocated_bytes_total", "Cumulative heap bytes allocated.", gauges.allocatedBytes());
        ...
        if (!heartbeats.isEmpty()) {
            sb.append("# HELP blackbox_heartbeat_last_beat_millis Unix timestamp of the last heartbeat, in milliseconds, per scope.\n");
            sb.append("# TYPE blackbox_heartbeat_last_beat_millis gauge\n");
            heartbeats.entrySet().stream()
                .sorted(Map.Entry.comparingByKey())
                .forEach(e -> sb.append("blackbox_heartbeat_last_beat_millis{scope=\"")
